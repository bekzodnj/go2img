...
type Action =
  | {
      type: "SET_POLYGONS";
      payload: { polygons: Polygon[]; shouldUpdateHistory?: boolean };
    }
...
export function PolygonProvider({
  ...
  const [state, dispatch] = useReducer(polygonReducer, {
    ...
    present: {
      ...
      activePolygonIndex: initialPolygons?.length
        ? initialPolygons.length - 1
        : 0,
    },
  });
  ...
  const setPolygons = useCallback(
    (polygons: Polygon[], shouldUpdateHistory = true) => {
      dispatch({
        type: "SET_POLYGONS",
        payload: { polygons, shouldUpdateHistory },
      });
    },
    [],
  );
